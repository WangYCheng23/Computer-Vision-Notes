...
def train(net, data_loader, loss_dict, optimizer, scheduler,logger, epoch, metric_dict, use_aux):
    ...
    for b_idx, data_label in enumerate(progress_bar):
        ...
        if hasattr(progress_bar,'set_postfix'):
            ...
            progress_bar.set_postfix(epoch = '%d'%epoch, loss = '%.3f' % float(loss.item()),
...
if __name__ == "__main__":
    ...
    print('args:', args)
    print('cfg:', cfg)
    work_dir = os.path.join(os.getcwd(), 'logs')
    ...
    train_loader, cls_num_per_lane = get_train_loader(cfg.batch_size, args.data_root, cfg.griding_num, cfg.dataset, cfg.use_aux, distributed, cfg.num_lanes)
    ...
    for epoch in range(resume_epoch, cfg.epoch):
        ...
        onepoch_loss = train(net, train_loader, loss_dict, optimizer, scheduler,logger, epoch, metric_dict, cfg.use_aux)
        if onepoch_loss<min_loss:
            torch.save(net.state_dict(), '/project/train/models/best.pth')
