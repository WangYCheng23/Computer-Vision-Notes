...
def get_tusimple_list(root, label_list):
    ...
    '''
    label_json_all, names = [], []
    for json_name in label_list:
        json_path = os.path.join(root, json_name)
        label_json = [json.loads(line) for line in open(json_path).readlines()]
        label_json_all += label_json
        name_ = [os.path.splitext(json_name)[0] + '.jpg'] * len(label_json)
        names += name_
...
def generate_segmentation_and_train_list(root, line_txt, names):
    ...
    for i in tqdm.tqdm(range(len(line_txt))):
        ...
        srcimg = cv2.imread(os.path.join(root, names[i]))
        label_path = names[i][:-3] + 'png'
        label = np.zeros((srcimg.shape[0], srcimg.shape[1]), dtype=np.uint8)
        bin_label = [0, 0, 0, 0]
...
def get_args():
    ...
    parser.add_argument('--data_root', required=True, help='The root of the Tusimple dataset')
    return parser
...
if __name__ == "__main__":
    ...
    label_list = list(filter(lambda x: x.endswith('.json'), os.listdir(args.data_root)))
    # training set
    names, line_txt = get_tusimple_list(args.data_root, label_list)
    # generate segmentation and training list for training
    generate_segmentation_and_train_list(args.data_root, line_txt, names)
    
    # testing set
    # names,line_txt = get_tusimple_list(args.data_root, ['test_tasks_0627.json'])
    # # generate testing set for testing
    # with open(os.path.join(args.data_root,'test.txt'),'w') as fp:
    #     for name in names:
    #         fp.write(name + '\n')
